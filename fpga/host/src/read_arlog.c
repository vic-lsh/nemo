...
int fpga_regs(bool poll) {
    ...
    /*
    unsigned long arlog_p1 = register_space[2730];
    unsigned long arlog_p2 = register_space[2731];
    printf("arlog: %lx %lx\n", arlog_p2, arlog_p1);
    */
    signal(SIGINT, sig_handler);
    if (poll) {
        printf("polling logs, quit with ^C\n");
        poll_ar_logs(register_space);
    } else {
        printf("emptying logs\n");
        empty_ar_logs(register_space);
    }
...
int main(int argc, char** argv) {
    ...
    // usage: ./a.out {p,e} {sync=0,1} {file}
    // p = poll, e = empty
    if (argc <= 2 || argc > 4) {
        fprintf(stderr, "Usage: ./a.out [p,e] [0,1] {file}\n");
        exit(1);
    }
    if (argv[2][1] != 0) {
        fprintf(stderr, "Usage: ./a.out [p,e] [0,1] {file}\n");
    ...
    bool empty_regs = argc == 1 || argv[1][0] != 'p';
    fpga_regs(!empty_regs);
    fclose(fil);
