...
#ifndef HEMEM_SHARED_BITOPS_H
#define HEMEM_SHARED_BITOPS_H
 ...
 */
inline int clz64(uint64_t val) { return val ? __builtin_clzll(val) : 64; }
 ...
 * undefined if the value is zero.
 */
inline int ctz64(uint64_t val) { return val ? __builtin_ctzll(val) : 64; }

/**
 * ctpop64 - count the population of one bits in a 64-bit value.
 * @val: The value to search
 */
inline int ctpop64(uint64_t val) { return __builtin_popcountll(val); }

#define clzl(x) clz64(x)
#define ctzl(x) ctz64(x)
