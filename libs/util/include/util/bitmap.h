 ...
 * bitmap_set_atomic(dst, pos, nbits)           Set specified bit area with
 * atomic ops bitmap_clear(dst, pos, nbits)                Clear specified bit
 * area bitmap_test_and_clear_atomic(dst, pos, nbits)    Test and clear area
 ...
 */
#define DECLARE_BITMAP32(name, bits) uint32_t name[BITS_TO_U32S(bits)]

#define small_nbits(nbits) ((nbits) <= (long)BITS_PER_LONG)
...
static inline long bitmap_count_one_with_offset(const unsigned long *bitmap,
    ...
    const unsigned long *bitmap_start = bitmap + aligned_offset / BITS_PER_LONG;
