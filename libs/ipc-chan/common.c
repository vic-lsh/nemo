...
int __recv_fds(int sock, int *fds, int nr_fds, void *data, unsigned ch_size,
    ...
    for (i = 0; i < nr_fds; i += min_fd) {
        ...
        if (ret <= 0) return ret ? __sys_err(ret) : -ENOMSG;

        cmsg = CMSG_FIRSTHDR(&fdset.hdr);
        if (!cmsg || cmsg->cmsg_type != SCM_RIGHTS) return -EINVAL;
        if (fdset.hdr.msg_flags & MSG_CTRUNC) return -ENFILE;
        ...
        if (unlikely(min_fd <= 0)) return -EBADFD;

        memcpy(&fds[i], cmsg_data, sizeof(int) * min_fd);
        if (data) data += ch_size * min_fd;
    }
